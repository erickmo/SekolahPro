...
const SEVERITY_OPTIONS: { value: CaseSeverity | ''; label: string }[] = [
  ...
  ...Object.entries(CASE_SEVERITY_LABELS).map(([value, label]) => ({
    value: value as CaseSeverity,
    label,
  })),
]
...
function getStatusClass(status: CaseStatus): string {
  switch (status) {
    ...
    case 'referred': return styles.badgeReferred
...
export default function StudentCounselingPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Kasus konseling berhasil dihapus')
      queryClient.invalidateQueries({ queryKey: [QK.counselingCase] })
    },
    onError: () => {
      toast.error('Gagal menghapus kasus konseling')
    },
  })
  ...
  const hasFilters = search !== '' || category !== '' || severity !== '' || status !== ''
  ...
  function handleDelete(item: CounselingCase) {
    if (!confirm(`Hapus kasus "${item.caseNo} - ${item.title}"?`)) return
    ...
    <div className={styles.root}>
      ...
      <div className={styles.filterRow}>
        ...
        <select
          className={styles.select}
          value={severity}
          onChange={(e) => { setSeverity(e.target.value as CaseSeverity | ''); setPage(1) }}
          ...
          <table className={styles.table}>
            <thead>
              <tr>
                ...
                <th className={styles.th}>Tanggal Dibuka</th>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={9} className={styles.emptyCell}>
                    <div className={styles.emptyState}>
                      ...
                      <p className={styles.emptySubtitle}>Belum ada kasus konseling yang tercatat.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                items.map((item) => (
                  <tr key={item.id} className={styles.row}>
                    ...
                    <td className={styles.td}>{CASE_CATEGORY_LABELS[item.category] ?? item.category}</td>
